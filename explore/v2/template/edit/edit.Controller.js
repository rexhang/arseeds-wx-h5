    ...
    $scope.$on('$viewContentLoaded', () => {
        ...
        if ( !token ){
            ...
            if( editId == 'add' ){
                ...
                // 定义上传识别图函数
                let resource3dUploader = (URL, DATA, from) => {
                    jQuery.ajax({
                        url: URL,
                        type: 'post',
                        dataType: 'json',
                        ...
                        success: function(data) {
                            if(data.code == 200){
                                if(from === 'android'){
                                    androidResourceUrl = data.data.file_url;
                                    $scope.android_3dfileName = data.data.file_name;
                                    $scope.android_3dfileSize = data.data.file_size;
                                    $scope.$apply();
                                } else{
                                    iosResourceUrl = data.data.file_url;
                                    $scope.ios_3dfileName = data.data.file_name;
                                    $scope.ios_3dfileSize = data.data.file_size;
                                    $scope.$apply();
                                }
                            } else{
                                layer.msg(data.msg, {
                ...
                // android ios 3d resource upload
                $scope.upload3dForAndroid = function(fileInfo){
                    layer.msg('文件上传中...', {
                      icon: 16
                      ,shade: [0.5,'#000']
                      ,time: 500000
                    ...
                    var maxSize = 10;
                    var acceptFileType = 'x-zip-compressed';
                    if(true_type === acceptFileType){
                        if(true_size > maxSize){
                            var msg = `您上传的文件大小[${parseInt(true_size)}M],不符合规范,仅允许${maxSize}M以下的文件上传！`;
                            layer.closeAll();
                            layer.alert(msg, {icon: 0});
                        } else {
                            // 【文件格式】、【大小】的验证通过
                            var uploadData = new FormData(); // 发送的数据对象
                            uploadData.append('type', '3d');
                            var thisFileObj = document.getElementById('androidResource').files;
                            uploadData.append('file', thisFileObj[0]);
                            resource3dUploader(`${baseURL}/explore2/img-show/upload`, uploadData, 'android');
                        }
                    } else{
                        var msg = `您上传的文件格式为[*.${true_type}],不符合规范,仅允许zip格式文件的上传！`;
                        layer.closeAll();
                        layer.alert(msg, {icon: 0});
                    }
                }
                $scope.upload3dForIos = function(fileInfo){
                    layer.msg('文件上传中...', {
                      icon: 16
                      ,shade: [0.5,'#000']
                      ,time: 500000
                    ...
                    if(true_type === acceptFileType){
                        if(true_size > maxSize){
                            ...
                            var uploadData = new FormData(); // 发送的数据对象
                            uploadData.append('type', '3d');
                            var thisFileObj = document.getElementById('iosRsource').files;
                            uploadData.append('file', thisFileObj[0]);
                            resource3dUploader(`${baseURL}/explore2/img-show/upload`, uploadData, 'ios');
                        }
                    } else{
                        var msg = `您上传的文件格式为[*.${true_type}],不符合规范,仅允许zip格式文件的上传！`;
                        layer.closeAll();
            ...
            jQuery('#ctrl-center > .nav-item').click(function(){
                ...
                jQuery('#group-box').children('div').eq(_index).show().siblings().hide();
            });
            ...
            var uploader = WebUploader.create({
                ...
                server: `${baseURL}/explore2/img-show/upload`,
                // 选择文件的按钮。可选。
            ...
            $scope.uploadMovmp4 = (fileInfo) => {
                ...
                if(fileSize > MaxSize){
                    ...
                    postVideo(baseURL+'/explore2/img-show/upload', videoData);
                }
            }
            ...
            editor.customConfig.uploadImgServer = baseURL + '/explore2/img-show/upload';
            ...
            $scope.createNew = function(){
                ...
                let isType = ''; // mix video 3d
                if(jQuery('#ctrl-center > .nav-item').eq(0).hasClass('sel')){
                    isType = 'mix';
                } else if(jQuery('#ctrl-center > .nav-item').eq(1).hasClass('sel')){
                    isType = 'video';
                } else{
                    isType = '3d';
                }
                ...
                if(isType === 'mix'){
                    ...
                    isResult = `${androidResourceUrl}|${iosResourceUrl}`; // 取视频地址
                }
                ...
                if(isType == '3d'){
                    isAndroid      = androidResourceUrl;
                    isAndroid_size = $scope.android_3dfileSize;
                    isIos          = iosResourceUrl;
                    isIos_size     = $scope.ios_3dfileSize;
                }
                ...
                let postData = {
                    ...
                    android: isAndroid,
                    android_size: isAndroid_size,
                    ios: isIos,
                    ios_size: isIos_size
                };
                console.warn(postData);
                let posttingData = ()=> {
                    serverConnect.__post(`${baseURL}/explore2/img-show/create`, postData, {}).success(function(data){
                        if(data.code == '200'){
                            layer.msg('恭喜!!数据新建成功', {
                                time: 1100
                            });
                ...
                if(isRecognition_img && isType && token && isResult){
                    if(isType === 'video'){
                        ...
                        if(isType === '3d'){
                            if(isAndroid && isAndroid_size && isIos && isIos_size){
                                posttingData();
                            } else{
                                layer.closeAll();
                                layer.alert('请检查3d模型Android和IOS是否都已经上传', {icon: 0});
                            }
                        } else{
                            posttingData();
                        }
