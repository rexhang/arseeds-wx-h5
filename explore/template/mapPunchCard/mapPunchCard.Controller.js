...
function mapPunchCardController($rootScope, $scope, URL, serverConnect, $interval, $state){
    ...
    $scope.$on('$viewContentLoaded', () => {
        ...
        class Card{
            constructor() {}
            getCardMaps(){
                serverConnect.__get(baseURL + '/wx/user/sign-point', {playing_id: is_playing_id, openid: openid }).success(function(data){
                    if (data.code == 200) {
            ...
            function onComplete(data){
                ...
                var converts = new AMap.convertFrom(lngLatTotal, 'gps', function(info, result){
                    ...
                    if(parseInt(rice) > parseInt(is_radius)){
                        ...
                        if( is_sign == 1 ){
                            // 多次打卡
                            $scope.zaicidaka = true; // 显示优惠券
                            $scope.cardIds = is_coupon_href; // 优惠券地址
                            layer.closeAll('loading');
                            $scope.$apply();
                        } else{
                            var getJson = {
                            ...
                            // 打卡接口
                            serverConnect.__get(baseURL + '/wx/user/add-sign', getJson).success(function(data){
                                if( data.code == 200 ){
                                    // 积分
                                    var jifenJson = {
                                        openid: openid,
                                        playing_id: is_playing_id,
                                        serial_id: is_node_id,
                                        score: is_point
                                    }
                                    // 积分接口
                                    serverConnect.__get(baseURL + '/wx/user/playing', jifenJson).success(function(data_result){
                                        if(data_result.code == 200){
                                            console.log('已积分');
                                        }
                                        $scope.dakachenggong = true; // 显示打卡成功
                                        is_coupon_href = is_coupon_href || ''; // 存在不存在取空
                                        $scope.cardId = is_coupon_href; // 优惠券地址
                                        $scope.$apply();
                                    }).error(function(data,status,headers,config){
                                        layer.msg('积分接口 连接错误', {
                                            time: 600
